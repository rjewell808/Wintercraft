...
public class ConnectionHandler {
	...
	public void onServerJoin(PlayerEvent.PlayerLoggedInEvent event){
		...
		event.player.addChatMessage(new ChatComponentText("\u00a7n\u00a7lWintercraft Version:\u00a7r\u00a7b\u00a7o " + WintercraftReference.VERSION));
		event.player.addChatMessage(new ChatComponentText(""));
		
		for(int i=0;i<teamUsername.length;i++)
		{
			if(teamUsername[i].equals(event.player.getDisplayName().getUnformattedText()))
			{
			...
			{
				event.player.addChatMessage(new ChatComponentText("Welcome " + event.player.getDisplayName().getUnformattedText() + " to Wintercraft!"));	
			}
		}
		...
		if(userSpecial != true)
		{
			...
			{
				if(betaMembers[i].equals(event.player.getDisplayName().getUnformattedText()))
				{
					userSpecial = true;
				}
			}
